...
function productTabsTree($style_code = '', $script = TRUE)
{
	...
	$cs->load->model('masters/product_tab_model');
	//----- รายการที่ติ๊ก
	$se = array('3' => 3, '8' => 8, '10' => 10); //$cs->product_tab_model->getStyleTabsId($style_code);

	//------- รายการที่ต้อง expan
	$parent = array('3' => 3); //$cs->product_tab_model->getParentTabsId($style_code);

	$sc	= '<ul class="tree">';
	$sc .= '<li>';
	$sc .= '<i class="fa fa-minus-square-o" id="catbox-0" onClick="toggleTree(0)"></i>';
	$sc .= '<label class="padding-10">';
	$sc .= 	'<input type="checkbox" class="ace margin-right-10" checked />';
	$sc .=  '<span class="lbl">  SUGGESTION</span>';
	$sc .= '</label>';
	$sc .= '<ul id="catchild-0">';
	$qs = $cs->db->query("SELECT * FROM product_tab WHERE id_parent = 0");
	$i = 1;
