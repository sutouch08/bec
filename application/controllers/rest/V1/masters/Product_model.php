...
class Product_model extends REST_Controller
{
  ...
  public function __construct()
  {
	...
	public function get_get($id = NULL)
	{
		...
		{
			$ds = $this->product_model_model->get_all();
  ...
  public function create_post()
  {
    ...
    $json = file_get_contents("php://input");
	  ...
	  if($sc === TRUE)
	  {
		  $cr = $this->product_model_model->get($ds->id);

		  if(empty($cr))
		  {
				if( ! $this->product_model_model->is_exists_name($ds->name))
				{
					$arr = array(
						'id' => $ds->id,
						'name' => $ds->name
		...
		if($sc === TRUE)
		{
			...
			$this->logs_model->logs_product_model($logs);
	...
	public function update_post($id)
  {
    ...
    $json = file_get_contents("php://input");
	  ...
	  if($sc === TRUE)
	  {
		  $cr = $this->product_model_model->get($id);

		  if( ! empty($cr))
		  {
				if( ! $this->product_model_model->is_exists_name($ds->name, $id))
				{
					$arr = array(
						'name' => $ds->name
					);
	...
	public function delete_post($id)
	{
		...
		if(! $this->product_model_model->delete($id))
		{
			$sc = FALSE;
