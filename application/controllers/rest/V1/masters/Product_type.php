...
class Product_type extends REST_Controller
{
    public $error;
		public $logs;
		public $log_json;
  ...
  public function __construct()
  {
	...
	public function get_get($id = NULL)
	{
		if( ! empty($id))
		{
			$rs = $this->product_type_model->get($id);
		...
		{
			$ds = $this->product_type_model->get_all();
  ...
  public function create_post()
  {
    ...
    $json = file_get_contents("php://input");
	  ...
	  if($sc === TRUE)
	  {
		  $cr = $this->product_type_model->get($ds->id);

		  if(empty($cr))
		  {
				if( ! $this->product_type_model->is_exists_name($ds->name))
				{
					$arr = array(
						'id' => $ds->id,
						'name' => $ds->name
		...
		if($sc === TRUE)
		{
			...
			$this->logs_model->logs_product_type($logs);
	...
	public function update_post($id)
  {
    ...
    $json = file_get_contents("php://input");
	  ...
	  if($sc === TRUE)
	  {
		  $cr = $this->product_type_model->get($id);

		  if( ! empty($cr))
		  {
				if( ! $this->product_type_model->is_exists_name($ds->name, $id))
				{
					$arr = array(
						'name' => $ds->name
					);
