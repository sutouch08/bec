...
class Customer_group extends REST_Controller
{
    public $error;
		public $logs;
		public $log_json;
  ...
  public function __construct()
  {
	...
	public function get_get($id = NULL)
	{
		...
		{
			$ds = $this->customer_group_model->get_all();
  ...
  public function create_post()
  {
    ...
    $json = file_get_contents("php://input");
	  ...
	  if(empty($ds))
	  {
		...
		if($sc === TRUE && empty($ds->GroupCode))
		{
			$sc = FALSE;
			$this->error = "Missing required parameter : GroupCode";
		}

		if($sc === TRUE && empty($ds->GroupName))
		{
			$sc = FALSE;
			$this->error = "Missing required parameter : GroupName";
		}


	  if($sc === TRUE)
	  {
		  $cr = $this->customer_group_model->get($ds->GroupCode);
		  ...
		  {
				$sc = FALSE;
				$this->error = "{$ds->GroupCode} already exists";
		  }
	  }
		...
		if($sc === TRUE)
		{
			...
			$this->logs_model->logs_customer_group($logs);
	...
	public function update_post($code)
  {
    ...
    $json = file_get_contents("php://input");
	  ...
	  if($sc === TRUE)
	  {
		  $cr = $this->customer_group_model->get($code);
