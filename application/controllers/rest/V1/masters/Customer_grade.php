...
class Customer_grade extends REST_Controller
{
    public $error;
		public $logs;
		public $log_json;
  ...
  public function __construct()
  {
	...
	public function get_get($id = NULL)
	{
		...
		{
			$ds = $this->customer_grade_model->get_all();
  ...
  public function create_post()
  {
    ...
    $json = file_get_contents("php://input");
	  ...
	  if($sc === TRUE)
	  {
		  $cr = $this->customer_grade_model->get($ds->id);

		  if(empty($cr))
		  {
				if( ! $this->customer_grade_model->is_exists($ds->name))
				{
					$arr = array(
						'id' => $ds->id,
						'name' => $ds->name
		...
		if($sc === TRUE)
		{
			...
			$this->logs_model->logs_customer_grade($logs);
	...
	public function update_post($id)
  {
    ...
    $json = file_get_contents("php://input");
	  ...
	  if($sc === TRUE)
	  {
		  $cr = $this->customer_grade_model->get($id);

		  if( ! empty($cr))
		  {
				if( ! $this->customer_grade_model->is_exists($ds->name, $id))
				{
					$arr = array(
						'name' => $ds->name
					);
	...
	public function delete_post($id)
	{
		...
		if(! $this->customer_grade_model->delete($id))
		{
			$sc = FALSE;
